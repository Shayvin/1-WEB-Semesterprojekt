...
if(!isset($_SESSION["username"])) // Wenn User nicht eingeloggt
{
  header("Location: ../index.php"); // Leite weiter an index.php
}
?>
<!DOCTYPE html>
<html lang="en">
...
if(isset($_GET["id"])) // Wenn get variable in der URL gesetzt
{
  if(!empty($_GET["id"])) // & wenn sie nicht leer ist
  {
      ...
      {
          $stmt = $mysql->prepare("UPDATE accounts SET SALUTATION = :salutation, FIRST_NAME = :firstname, LAST_NAME = :lastname, EMAIL = :email WHERE USERNAME = :id"); // Prepare SQL Statement
          $stmt->execute(array(":salutation" => $_POST["salutation"], ":firstname" => $_POST["firstname"], ":lastname" => $_POST["lastname"], ":email" => $_POST["email"],  ":id" => $_GET["id"])); // Setze lokale Variablen gleich der POST/GET und führe aus
          ?>
      ...
      ?>
            ...
            <form action="profil.php?id=<?php echo $_GET["id"] ?>" method="post">
